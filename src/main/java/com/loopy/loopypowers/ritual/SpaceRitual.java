...
import com.loopy.loopypowers.network.payload.SpaceRitualTickPayload;
...
public class SpaceRitual implements RitualInterface {
    ...
    public boolean tick(ServerLevel world) {
        ...
        PacketDistributor.sendToPlayersTrackingEntityAndSelf(sp, new SpaceRitualTickPayload(sp.getId(), ticks));
    ...
    private void tickStage4ServerOnly(ServerPlayer sp, ServerLevel world, int t) {
        ...
        if (progress < 0.80f) {
            sp.addEffect(new MobEffectInstance(MobEffects.LEVITATION, 10, 1, true, false, false));
        }
    ...
    private void onComplete(ServerPlayer sp, ServerLevel world) {
        Supplier<PowerInterface> factory = SPACE_POWERS.get(sp.getRandom().nextInt(SPACE_POWERS.size()));
